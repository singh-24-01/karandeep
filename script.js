...
(function(){
  ...
  function renderView(){
    ...
    if (addNewsBtn) {
      addNewsBtn.classList.toggle('hidden', !isOwner);
    }
 ...
 auth.onAuthStateChanged(user => {
  ...
  if (newsList) {
    db.collection("news")
      ...
      .onSnapshot(snap => {
        newsList.innerHTML = "";
        if (snap.empty) {
          newsList.textContent = "(aucune actualité pour l’instant)";
          return;
        }
        ...
        snap.forEach(doc => {
          ...
          let inner = `
            <div class="news-title">${data.title || "Info"}</div>
            <div class="news-body">${(data.body || "").replace(/\n/g, "<br>")}</div>
            <div class="news-meta">
        ...
        if (currentUserUid === OWNER_UID) {
          const deleteButtons = newsList.querySelectorAll('.history-delete-btn');
          deleteButtons.forEach(btn => {
  ...
  if (addNewsBtn) {
    addNewsBtn.addEventListener('click', async () => {
      if (currentUserUid !== OWNER_UID) {
        alert("Tu n’as pas les droits pour ajouter une actualité.");
        return;
      }
      const title = prompt("Titre de l'actualité :", "Changement de salle");
      if (!title) return;
      const body = prompt("Texte (détails) :", "Cours déplacé en salle Binet…") || "";
